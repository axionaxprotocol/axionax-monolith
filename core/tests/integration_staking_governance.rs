...
async fn test_governance_uses_stake_weight() {
    ...
    governance.vote("validator1".to_string(), proposal_id, VoteOption::For, v1_stake).await.unwrap();
    governance.vote("validator2".to_string(), proposal_id, VoteOption::Against, v2_stake).await.unwrap();
...
async fn test_slashing_reduces_voting_power() {
    ...
    // Stake
    staking.stake("validator1".to_string(), 10000).await.unwrap();
    
    let initial_power = staking.get_validator("validator1").await.unwrap().voting_power();
...
async fn test_delegation_increases_voting_power() {
    ...
    // Delegator delegates
    staking.delegate("delegator1".to_string(), "validator1".to_string(), 2000).await.unwrap();
    staking.delegate("delegator2".to_string(), "validator1".to_string(), 3000).await.unwrap();
...
async fn test_full_governance_flow() {
    ...
    governance.vote("voter1".to_string(), proposal_id, VoteOption::For, 3000).await.unwrap();
    governance.vote("voter2".to_string(), proposal_id, VoteOption::For, 1500).await.unwrap();
    governance.vote("voter3".to_string(), proposal_id, VoteOption::Against, 500).await.unwrap();

    // Advance past voting period
    governance.set_current_block(20).await;

    // Finalize
    let status = governance.finalize_proposal(proposal_id, 10000).await.unwrap();
...
async fn test_unstaking_lock_period() {
    ...
    staking.unstake("validator".to_string(), 5000).await.unwrap();
