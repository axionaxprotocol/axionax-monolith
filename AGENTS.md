...
|---------|-----------|------|-------|
| **Rust Core (build/test)** | `cd core && cargo build --workspace` / `cargo test --workspace` | — | Primary dev workflow; 257 tests, all pass |
| **Mock RPC** | `cd ops/deploy/mock-rpc && node server.js` | 8545 (HTTP), 8546 (WS) | Lightweight mock JSON-RPC; no Rust compile needed |
| **Python DeAI tests** | `cd core/deai && python3 -m pytest . -v --tb=short --ignore=tests` | — | 4 pass, 2 skip (bridge + Docker sandbox) |

### Non-obvious caveats

- **Rust MSRV:** The `Cargo.toml` declares `rust-version = "1.83"`, but transitive dependencies (e.g. `time-macros`) require Rust **1.85+** due to `edition2024`. Run `rustup update stable` if builds fail.
- **System deps:** `libssl-dev`, `libclang-dev`, and `python3-dev` must be installed for the Rust build (RocksDB, libp2p, PyO3 bindings).
- **Clippy:** Pre-existing deny-level warnings in `core/core/metrics/` (approximate `PI` constant) and bench compilation errors in `consensus_bench.rs`. Use `cargo clippy --workspace --lib --tests` (omit `--all-targets`) to skip bench issues.
- **Formatting:** `cargo fmt --all -- --check` shows a pre-existing diff in `tools/faucet/src/main.rs`. This is not caused by agent changes.
